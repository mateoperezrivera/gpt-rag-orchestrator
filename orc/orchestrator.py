...
async def run(conversation_id, ask, client_principal,files=None):
    ...
    async with ChainedTokenCredential(
        ...
        async with CosmosClient(AZURE_DB_URI, credential=credential) as db_client:
            ...
            answer_dict = await code_orchestration.get_answer(history, security_ids,conversation_id,credential,files)
